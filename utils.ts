import {Weighable} from "./weighable.ts";
import {ScaleOrChord} from "./ScaleOrChord.ts";

export function getRandomElementWithWeight(elements: Array<Weighable>): Weighable {
...
export function getProbability(element: Weighable, elements: Array<Weighable>): string {
    const totalWeight = elements.reduce((sum, element) => sum + element.getWeight(), 0);
...
export function updateTable(scaleOrChords: Array<ScaleOrChord>, sortBy: (scaleOrChord: ScaleOrChord) => number) {
...
export function updateOccurrencesAndAverageTime(scaleOrChord: ScaleOrChord, duration: number) {
